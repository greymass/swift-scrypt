...
int libscrypt_check_legacy(char *mcf, const char *password)
{
	...
	*/
	...
	if(memcmp(mcf, SCRYPT_MCF_ID_LEGACY, 3) != 0)
	{
	...
	*/
	...
	retval = libscrypt_b64_decode_legacy(tok, (unsigned char*)salt, sizeof(salt));
	if (retval < 1)
		return -1;

	retval = libscrypt_scrypt_legacy((uint8_t*)password, strlen(password), salt,
	...
	if(slow_equals_legacy(tok, outbuf) == 0)
		return 0;
